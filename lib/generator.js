...
function twitter_filter_after_post_render(hexo, data) {
    ...
    if (!data.slug) {
        logger.warn("Skipping invalid article " + data.title);
        return;
    }
    let tNotif = dbNotif.get('social_notifs').find({
        slug: data.slug
    }).value();
    logger.info('[Hexo Twitter] Article in database(slug %s) : %d', data.slug, tNotif);
...
function twitter_generate(locals) {
    ...
    logger.info("[Hexo Twitter] Notifying twitter for new or updated posts (%d)", social.postsToNotify.length);
