...
function init(hexo) {
    ...
    if (hexo.config.twitter.database_renew) {
        ...
        logger.warn("[Hexo Twitter] Actual posts " + dbNotif.get('twitter_notifs').size());
    }


    logger.debug("[Hexo Twitter] Configuration %j", hexo.config.twitter);
    // Component initialization
    let twit = hexo.config.twitter

    hexo.locals.set('twitter', {
...
function twitter_filter_after_post_render(hexo, data) {
    ...
    const twitter = hexo.locals.get('twitter');
    const config = hexo.config.twitter;
    const dbNotif = twitter.dbNotif;

    let tNotif = dbNotif.get('twitter_notifs').find({
        slug: data.slug
    }).value();
    ...
    if (!tNotif) {
        ...
        dbNotif.get('twitter_notifs').push({
            id: data._id,
            slug: data.slug,
            date: data.date,
        ...
        twitter.postsToNotify.push(data);
    } else {
        ...
        if (!tNotif.updated || new Date(tNotif.updated) < data.updated.toDate()) {
            ...
            dbNotif.get('twitter_notifs').find({
                slug: data.slug
            }).assign({
                updated: data.updated
            }).value();
...
function twitter_generate(locals) {
    const twitterLocals = locals.twitter;
    logger.debug("[Hexo Twitter] Notifying twitter for new or updated posts (%d)", twitterLocals.postsToNotify.length);

    var configURL = twitterLocals.siteURL;

    twitterLocals.postsToNotify.forEach(
        ...
        element => {
            ...
            let TweetData = twitter.new_tweet(
                PostData,
                twitterLocals.config
            );
            ...
            twitter.send_tweet(TweetData, function(err, response) {
    ...
    twitterLocals.allimagesUrls = [];
    twitterLocals.postsToNotify = [];
